...
I1 = imread('ps/2020-09-22_100828.jpg'); 

word=111;
%word=222;
mysize=size(I1);
if numel(mysize)>2
    I=rgb2gray(I1); %将彩色图像转换为灰度图像
else
    I=I1;
end
%%%%figure(1),imshow(I); 
...
%figure(2);imshow(b);
%从上到下定位每一行第一个白点的行列数据,istart为有白点存在的第一行，jstart为有白点存在的第一列
istart=0;
start=zeros(m1,2);
for i=1:m1  
    for j=1:n1
...
%矫正istart,存在图像上侧出现单独白点情况
istart=start(1,1);
%if istart<100  istart=100;end
 % 水平膨胀 
num=20;%膨胀幅度
swell=round(num/2);
temp =zeros(m1,n1);
for i=1:m1               
    for j=swell:(n1-swell+1)
        for k=(-swell+1) : (swell-1)
            if b(i,(j+k))==1
                temp(i,j)=1;
                break;
            end
...
%figure(4), imshow(I3); 
% hough变换，获取旋转角度
rho = round(sqrt(m1^2 + n1^2)); % 获取ρ的最大值
theta = 1800; % 获取θ的最大值
...
for i = 1 : m1
   for j = 1 : n1
      if I3(i,j) == 1  % I3是边缘检测得到的图像
          for k = 1 : theta %角度
             ru = round(abs(i*cosd(0.1*k) + j*sind(0.1*k)));
...
if c <= 900
   rot_theta = (-c)/10;  % 确定旋转角度
else
    rot_theta = (1800 - c)/10; 
end

% fprintf('rot_theta=%f ',rot_theta);
% I4 = imrotate(b, rot_theta, 'crop');  % 对小图像进行旋转，校正图像
%figure(5), imshow(I4);
I5 = imrotate(I, rot_theta, 'crop');  % 对原图像进行旋转，校正图像
%%%figure(6), imshow(I5);
%去除图像左侧、右侧黑边
for i=1:m
    for j=1:round(jstart/2)        
        I5(i,j)=255;     
    end
    for j=round(n-jstart/2):n        
        I5(i,j)=255;     
    end
end
%去除图像上侧、下侧黑边
for j=round(jstart/2) :round(n-jstart/2)
    if i~=0 && j~=0
        for i=1:round(istart/2)     
            I5(i,j)=255;     
        end
        for i=round(m-istart/2):m  
            I5(i,j)=255;     
        end
    end
end
